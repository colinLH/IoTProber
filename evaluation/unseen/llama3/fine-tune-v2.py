...
def _parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Continue training from an existing LoRA adapter checkpoint "
             "(e.g. evaluation/unseen/llama3/results/checkpoint-14576) "
             "instead of starting from fresh LoRA weights.",
    )
    p.add_argument(
        "--num_train_epochs_override",
        type=float,
        default=None,
        help="Override TRAIN_EPOCHS (v2 continuation uses the same 1.0).",
    )
    p.add_argument(
        "--write_model_on_gate_failure",
        action="store_true",
        default=False,
        help="Save the adapter even if the built-in release gate fails.",
    )
    p.add_argument(
        "--save_steps",
        type=int,
        default=5000,
        help="Checkpoint every N optimizer steps (v2 continuation uses 200).",
    )
    p.add_argument(
        "--skip_builtin_calibration",
        action="store_true",
        default=False,
        help="Skip the in-process calibration/release-gate phase; the "
             "standalone 8-GPU sharded calibration runs afterwards. This also "
             "avoids the DDP phase where the original run NCCL-watchdog-timed-"
             "out (every rank re-ran the full sequential calibration).",
    )
    return p.parse_args()
...
def run_training(args=None):
    ...
    if args.init_adapter:
        # Continue from a previously trained adapter checkpoint instead of
        # fresh LoRA weights. PeftModel.from_pretrained loads the adapter's own
        # config + weights on top of the quantized base — do NOT call
        # get_peft_model() first (that would nest two PeftModels).
        from peft import PeftModel
        if is_main:
            print(f"Continuing training from adapter: {args.init_adapter}")
    ...
    training_args = TrainingArguments(
        ...
        num_train_epochs=(
            args.num_train_epochs_override
            if getattr(args, "num_train_epochs_override", None) is not None
            else TRAIN_EPOCHS
        ),
        ...
        logging_steps=10,
        save_strategy="steps",
        save_steps=getattr(args, "save_steps", 5000) or 5000,
        save_total_limit=1,
        eval_strategy="epoch",
    ...
    if is_main:
        ...
        if args.skip_builtin_calibration:
            # The standalone 8-GPU sharded calibration runs afterwards. Skipping
            # also avoids the phase that crashed the original run: every DDP
            # rank independently re-ran the full sequential calibration after
            # training, and the rank imbalance tripped the NCCL watchdog on
            # rank 7.
            print("[skip] Built-in calibration/gate disabled "
                  "(--skip_builtin_calibration); run the standalone calibration "
                  "afterwards.")
            metadata["release_gate"] = {
                "passed": None,
                "skipped": True,
                "reason": "deferred to standalone 8-GPU sharded calibration",
            }
        else:
            ...
            if not release_gate["passed"] and not getattr(
                args, "write_model_on_gate_failure", False
            ):
                raise RuntimeError(
                    f"Release gate failed; see {report_path}"
                )
            if not release_gate["passed"]:
                print("WARNING: release gate failed; saving adapter anyway "
                      "(--write_model_on_gate_failure).")
